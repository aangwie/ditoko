...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white shadow-sm sm:rounded-lg mb-6">
        <div class="p-6">
            <h3 class="text-lg font-semibold mb-4 text-ditoko-navy">Konfigurasi Google OAuth</h3>
            <form action="{{ route('admin.settings.update') }}" method="POST">
                @csrf @method('PUT')
                @foreach($googleSettings as $setting)
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ ucwords(str_replace('_', ' ', str_replace('google_', '', $setting->key))) }}</label>
                        @if(str_contains($setting->key, 'secret'))
                            <div class="relative">
                                <input type="password" name="settings[{{ $setting->key }}]" value="{{ old('settings.'.$setting->key, $setting->value) }}"
                                       ...
                                       placeholder="{{ $setting->description }}" id="{{ $setting->key }}_input">
                                <button type="button" onclick="toggleField('{{ $setting->key }}_input')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 hover:text-gray-700">
                ...
                @endforeach
                <div class="pt-6 border-t">
                    <button type="submit" class="w-full bg-ditoko-navy text-white px-6 py-3 rounded-lg hover:bg-blue-900">💾 Simpan Google OAuth</button>
                </div>
            </form>
            <script>
